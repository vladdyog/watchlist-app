...
const App: React.FC = () => {
  ...
  useEffect(() => { saveToStorage(DECK_KEY, deck); }, [deck]);
  useEffect(() => { saveToStorage(DECK_ENABLED_KEY, deckEnabled); }, [deckEnabled]);
  ...
  const handleMoviePicked = (movie: Movie) => {
    if (!deckEnabled) { setLastPick(movie); return; }
    setDeck((prev) => prev.some((m) => m.title === movie.title) ? prev : [...prev, movie]);
  };

  const handleWatchThis = (winner: Movie) => { setLastPick(winner); setDeck([]); };
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-bg)' }}>
      ...
      <header style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
        <div style={{ maxWidth: 672, margin: '0 auto', padding: '28px 24px' }}>
          ...
          <p style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)', marginTop: '6px', fontWeight: 500 }}>
      ...
      {/* ── Main ── */}
      <main style={{ flex: 1, maxWidth: 672, width: '100%', margin: '0 auto', padding: '40px 24px', display: 'flex', flexDirection: 'column', gap: '44px' }}>
        ...
        <section>
          ...
          />
          {error && (
            <p style={{ color: 'var(--color-danger)', fontSize: '0.9rem', marginTop: '10px', fontWeight: 500 }}>
              {error}
            </p>
          )}
        </section>
          ...
          <>
            ...
            <section>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
                ...
                <span style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)', fontWeight: 500 }}>
                  <span style={{ color: 'var(--color-text)', fontWeight: 700 }}>{filteredMovies.length}</span>
                  {' / '}
                  <span>{movies.length} films</span>
                </span>
              </div>
              <MovieFilters movies={movies} filters={filters} onChange={setFilters} />
            </section>
            ...
            <section>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
                ...
                <DeckToggle enabled={deckEnabled} onToggle={() => setDeckEnabled((v) => !v)} />
              </div>
              ...
              {deckEnabled && (
                <div style={{ marginTop: '12px' }}>
                  <MovieDeck
                    ...
                    onRemove={(m) => setDeck((prev) => prev.filter((w) => w.title !== m.title))}
                    onClear={() => setDeck([])}
                  />
                </div>
              )}
              ...
              {showDeckWinnerModal && lastPick && (
                <MovieModal movie={lastPick} onClose={() => setShowDeckWinnerModal(false)} />
              )}
            </section>
          </>
        )}
      ...
      <footer style={{ borderTop: '1px solid var(--color-border)', padding: '24px', textAlign: 'center' }}>
        <p style={{ fontSize: '0.8rem', color: 'var(--color-muted)', fontWeight: 500 }}>
          CueMovie · v{APP_VERSION} · © 2026 {AUTHOR}
        </p>
        <a
          href="https://www.flaticon.com/free-icons/clapper"
          style={{ fontSize: '0.75rem', color: 'var(--color-muted)', textDecoration: 'none', marginTop: '4px', display: 'block' }}
        >
